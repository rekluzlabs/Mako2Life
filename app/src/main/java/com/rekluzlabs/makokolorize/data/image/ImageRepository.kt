...
class ImageRepository(private val context: Context) {
    ...
    suspend fun saveProcessedBitmap(bitmap: Bitmap): Result<Uri> = withContext(Dispatchers.IO) {
        try {
            val sessionId = sessionCounter.incrementAndGet().toString()
            val dir = File(context.cacheDir, "restore/session_$sessionId")
            dir.mkdirs()
    ...
    suspend fun fetchLocalImages(): Result<List<Uri>> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun saveBitmap(bitmap: Bitmap, format: Bitmap.CompressFormat, quality: Int = 95): Result<Uri> = withContext(Dispatchers.IO) {
        try {
            val dir = File(context.filesDir, "exports")
            dir.mkdirs()
            val ext = if (format == Bitmap.CompressFormat.JPEG) "jpg" else "png"
            val file = File(dir, "makokolorize_${System.currentTimeMillis()}.$ext")
            FileOutputStream(file).use { out ->
